...
func (h *Handler) CreateShortURLHandler(rw http.ResponseWriter, r *http.Request) {
	...
	model := models.MainModel{ID: uuid, URLpair: models.Model{LongURL: string(body)}}
	// model := models.MainModel{ID: uuid, URLpair.LongURL: string(body)}
	// if !h.Storage.UUIDIsExist(uuid) {
	...
	model.URLpair.ShortURL = bURL + model.URLpair.ShortURL
	// mURL.ShortURL = bURL + mURL.ShortURL
	rw.Header().Set("Content-Type", "text/plain; charset=utf-8")
	rw.WriteHeader(http.StatusCreated)
	rw.Write([]byte(model.URLpair.ShortURL))
...
func (h *Handler) CreateInJSONShortURLHandler(rw http.ResponseWriter, r *http.Request) {
	...
	outObj := OutObj{Result: bURL + model.URLpair.ShortURL}
